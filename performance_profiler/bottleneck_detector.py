...
class Bottleneck:
    ...
    def __init__(self, location: str, line_no: int, severity: str,
                 metric_type: str, metric_value: float, description: str):
...
class BottleneckDetector:
    ...
    def print_bottlenecks(self):
        ...
        for category, items, emoji in [
            ...
            if items:
                ...
                if time_items:
                    print(f"\n  ⏱️  Time Bottlenecks:")
                    for item in sorted(time_items, key=lambda x: x.metric_value, reverse=True):
                        print(f"     {emoji} Line {item.line_no}: {item.description}")
                        print(f"        Time: {item.metric_value:.4f}s")
                
                if memory_items:
                    print(f"\n  💾 Memory Bottlenecks:")
                    for item in sorted(memory_items, key=lambda x: x.metric_value, reverse=True):
                        print(f"     {emoji} Line {item.line_no}: {item.description}")
                
                if call_items:
                    print(f"\n  🔄 Call Frequency Bottlenecks:")
                    for item in sorted(call_items, key=lambda x: x.metric_value, reverse=True):
                        print(f"     {emoji} Line {item.line_no}: {item.description}")
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'by_type': {
                ...
                'memory': len([b for b in self.bottlenecks if b.metric_type == "memory"]),
    ...
    def get_recommendations(self) -> List[str]:
        ...
        critical_time = [b for b in self.bottlenecks 
        ...
        critical_memory = [b for b in self.bottlenecks 
        ...
        high_calls = [b for b in self.bottlenecks 
